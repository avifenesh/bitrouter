...
- *(auth)* Prefer explicit provider key - ([7bccb55](https://github.com/bitrouter/bitrouter/commit/7bccb557f4a7051976a14182b1851d4040881161))
...
- Model_info/model_pricing fall back to top-level models map ([#441](https://github.com/bitrouter/bitrouter/pull/441)) - ([94400f4](https://github.com/bitrouter/bitrouter/commit/94400f40f368610c4b7457b7099feb5a9ec3b035))

### ⚙️ Miscellaneous Tasks

- *(providers)* Update models from models.dev ([#438](https://github.com/bitrouter/bitrouter/pull/438)) - ([4461ce9](https://github.com/bitrouter/bitrouter/commit/4461ce992ec4d91cd28b9ce80281307f3581a899))
...
- Cc, messages API and copilot compatibility ([#429](https://github.com/bitrouter/bitrouter/pull/429)) - ([daaa053](https://github.com/bitrouter/bitrouter/commit/daaa0536cbd0eb46a09ec0f8ffb0a6839caa1639))
...
- *(providers)* Update models from models.dev ([#396](https://github.com/bitrouter/bitrouter/pull/396)) - ([8169aeb](https://github.com/bitrouter/bitrouter/commit/8169aeb061640c011034b5aa65263dbc0b19648a))
...
- 402/MPP payments and the OWS+alloy wallet stack are now **baked into the `bitrouter` binary unconditionally**. The binary-level Cargo features `payments-tempo` / `payments-solana` have been renamed to `tempo` / `solana` and now act as **chain backend selectors** (at least one must be enabled; default: `tempo`). The default feature set no longer includes `solana` — enable it explicitly to compile in the Solana provider. `bitrouter-api` and `bitrouter-config` retain their `payments-tempo` / `payments-solana` feature names; only the binary's user-facing names changed.
- The operator-facing CLI surface — every `bitrouter` subcommand other than `serve` (`start`/`stop`/`status`/`restart`/`reload`/`route`/`wallet`/`key`/`tools`/`models`/`agents`/`agent-proxy`/`policy`/`auth`/`reset`), the setup wizard, and the daemon manager — is now gated behind the new `cli` Cargo feature on the `bitrouter` binary. `tui` now implies `cli`. The default feature set is `["cli","tui","sqlite","tempo","mcp","rest"]`, so existing users see no change. To build a minimal serve-only binary, use `--no-default-features --features tempo,sqlite,mcp,rest` (drops `dialoguer`, `libc`, the daemon manager, the setup wizard, and the OAuth device-flow helpers).
...
- Maintenance auto-fixes — 2026-04-21 ([#358](https://github.com/bitrouter/bitrouter/pull/358)) - ([a030c68](https://github.com/bitrouter/bitrouter/commit/a030c68a3c52f46cfe60b46c00fe48b172a1d701))
...
- *(providers)* Update models from models.dev ([#337](https://github.com/bitrouter/bitrouter/pull/337)) - ([587c76d](https://github.com/bitrouter/bitrouter/commit/587c76df8c4fcdbbc4627f276bf1b7bebced5f11))
- Update Cargo.lock dependencies - ([0000000](https://github.com/bitrouter/bitrouter/commit/0000000))


## [0.24.0](https://github.com/bitrouter/bitrouter/compare/v0.23.1...v0.24.0)
...
- *(copilot)* Request headers, Claude Anthropic routing, and Enterprise support ([#326](https://github.com/bitrouter/bitrouter/pull/326)) - ([95c6efc](https://github.com/bitrouter/bitrouter/commit/95c6efc9f8a888664c66f212562e50d376bb0ec6))
...
- ACP stdio proxy for agent-proxy CLI subcommand ([#316](https://github.com/bitrouter/bitrouter/pull/316)) - ([245f4f4](https://github.com/bitrouter/bitrouter/commit/245f4f41a08e245d45507bda7d19f9357577b440))
